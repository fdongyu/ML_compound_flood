...
if __name__ == "__main__":
    ...
    """
    step one: read numerical model output
    """
    ...
    ## model domain
    Nobs = 4
    interval = int(Nx/Nobs)

    ind_obs = np.arange(0,Nx,interval)[1:]
    t_obs_u = np.array([])
    x_obs_u = np.array([])
    u_obs = np.array([])
    for iobs in ind_obs:
    ...
    if exist_mode == 0:
        ...
        model.train(num_epochs = 80000)

        endtimer = time.time()
        print ('PINN Time elapsed: ', endtimer-starttimer)

        save_path = 'saved_model/PINN_uh_Telemac_Nobs{}.pickle'.format(Nobs)
