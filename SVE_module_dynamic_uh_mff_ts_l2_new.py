...
class SVE:
    ...
    def __init__(self, X_u_IC, X_h_IC, 
                      X_u_BC, X_h_BC,
                      X_u_obs, X_h_obs,
                      X_f, 
                      u_IC, h_IC,
        ...
        self.x_u_IC = X_u_IC[:,0:1]
        self.t_u_IC = X_u_IC[:,1:2]
        ...
        self.W2_t = tf.Variable(tf.random_normal([1, layers[0] // 2], dtype=self.DTYPE) * 10.0,
                               dtype=self.DTYPE, trainable=False)
        ...
        self.x_u_IC_tf = tf.placeholder(self.DTYPE, shape=[None, self.x_u_IC.shape[1]])
        self.t_u_IC_tf = tf.placeholder(self.DTYPE, shape=[None, self.t_u_IC.shape[1]])        
        self.u_IC_tf = tf.placeholder(self.DTYPE, shape=[None, self.u_IC.shape[1]])
        ...
        if ExistModel == 0:
            ...
            self.adaptive_constant_ics_u_tf, \
            ...
            print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_u_val: {:.3f}, constant_ics_h_val: {:.3f}, constant_obs_u_val: {:.3f}, constant_obs_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_tf, self.adaptive_constant_bcs_h_tf, self.adaptive_constant_ics_u_tf, self.adaptive_constant_ics_h_tf, self.adaptive_constant_obs_u_tf, self.adaptive_constant_obs_h_tf))

            self.W1_t, self.W2_t, self.W1_x = self.load_wmff(wmffDir)
        
        # physics informed neural networks
        self.u_pred, self.h_pred = self.net_uh(self.x_u_tf, self.t_u_tf)
        self.u_IC_pred, self.h_IC_pred = self.net_uh(self.x_u_IC_tf, self.t_u_IC_tf) 
        ...
        self.loss_IC_u = tf.reduce_mean(tf.square(self.u_IC_tf - self.u_IC_pred)) 
        self.loss_IC_h = tf.reduce_mean(tf.square(self.h_IC_tf - self.h_IC_pred))
        self.loss_ICs = self.adaptive_constant_ics_u_tf * self.loss_IC_u \
                        + self.adaptive_constant_ics_h_tf * self.loss_IC_h        

        # L2
        lambda_reg = 0.005
        #self.loss_l2 = (self.cal(self.weights)) / (len(h_obs)+len(S)) * lambda_reg
        self.loss_l2 = self.cal(self.weights) * lambda_reg

        self.loss = self.loss_f + self.loss_BCs + self.loss_ICs + self.loss_l2
        ...
        self.dict_gradients_ics_u_layers = self.generate_grad_dict(layers)
        ...
        for i in range(len(layers)-1):
            ...
            self.grad_ics_u.append(tf.gradients(self.loss_IC_u, self.weights[i])[0])
        ...
        self.adpative_constant_ics_u_list = []
        self.adpative_constant_ics_u_log = []
        ...
        for i in range(len(layers)-1):
            ...
            self.adpative_constant_ics_u_list.append(
                tf.reduce_max(tf.abs(self.grad_res[i])) / tf.reduce_mean(tf.abs(self.grad_ics_u[i])))
        ...
        self.adaptive_constant_ics_u = tf.reduce_max(tf.stack(self.adpative_constant_ics_u_list))
    ...
    def save_gradients(self, tf_dict):
        ...
        for i in range(num_layers):
            if self.useObs:
                grad_res_value, grad_bcs_u_value, grad_bcs_h_value, grad_ics_u_value, grad_ics_h_value, grad_obs_u_value, grad_obs_h_value = \
                self.sess.run([self.grad_res[i], self.grad_bcs_u[i], self.grad_bcs_h[i], self.grad_ics_u[i], self.grad_ics_h[i], self.grad_obs_u[i], self.grad_obs_h[i]], feed_dict=tf_dict)
            else:
                grad_res_value, grad_bcs_u_value, grad_bcs_h_value, grad_ics_u_value, grad_ics_h_value =\
                self.sess.run([self.grad_res[i], self.grad_bcs_u[i], self.grad_bcs_h[i], self.grad_ics_u[i], self.grad_ics_h[i]], feed_dict=tf_dict)
            ...
            self.dict_gradients_ics_u_layers['layer_' + str(i + 1)].append(grad_ics_u_value.flatten())
    ...
    def cal(self, W):
        f = 0
        for each in W:
            f += tf.reduce_sum(tf.square(each))
        return f
    ...
    def callback(self, loss, loss_f_c, loss_f_m, loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h):
        ...
        self.loss_IC_u_log.append(loss_IC_u)
    ...
    def train(self, num_epochs):
        ...
        tf_dict = {self.x_u_IC_tf: self.x_u_IC, self.t_u_IC_tf: self.t_u_IC, self.u_IC_tf: self.u_IC, 
                   self.x_h_IC_tf: self.x_h_IC, self.t_h_IC_tf: self.t_h_IC, self.h_IC_tf: self.h_IC, 
                   ...
                   self.adaptive_constant_ics_u_tf: self.adaptive_constant_ics_u_val,
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                if self.useObs: 
                    loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h, loss_f_c, loss_f_m, loss_l2 = \
                        self.sess.run([self.loss_BC_u, self.loss_BC_h, self.loss_IC_u, self.loss_IC_h, self.loss_obs_u, self.loss_obs_h, self.loss_f_c, self.loss_f_m, self.loss_l2], tf_dict)
                    print ('Loss_BC_u: %.3e, Loss_BC_h: %.3e, Loss_IC_u: %.3e, Loss_IC_h: %.3e, Loss_obs_u: %.3e, Loss_obs_h: %.3e, Loss_f_c: %.3e, Loss_f_m: %.3e, Loss_L2: %.3e, Error u: %.3e, Error h: %.3e'
                            %(loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h, loss_f_c, loss_f_m, loss_l2, error_u, error_h))
                else:
                    loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_f_c, loss_f_m = \
                        self.sess.run([self.loss_BC_u, self.loss_BC_h, self.loss_IC_u, self.loss_IC_h, self.loss_f_c, self.loss_f_m], tf_dict)
                    print ('Loss_BC_u: %.3e, Loss_BC_h: %.3e, Loss_IC_u: %.3e, Loss_IC_h: %.3e, Loss_f_c: %.3e, Loss_f_m: %.3e, Error u: %.3e, Error h: %.3e'
                            %(loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_f_c, loss_f_m, error_u, error_h))
                ...
                adaptive_constant_ics_u_val = self.sess.run(self.adaptive_constant_ics_u, tf_dict)
                self.adaptive_constant_ics_u_val = adaptive_constant_ics_u_val * \
                                            (1.0 - self.beta) + self.beta * self.adaptive_constant_ics_u_val
                self.adpative_constant_ics_u_log.append(self.adaptive_constant_ics_u_val)
                ...
                if self.useObs:
                    print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_u_val: {:.3f}, constant_ics_h_val: {:.3f}, constant_obs_u_val: {:.3f}, constant_obs_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_val, self.adaptive_constant_bcs_h_val, self.adaptive_constant_ics_u_val, self.adaptive_constant_ics_h_val, self.adaptive_constant_obs_u_val, self.adaptive_constant_obs_h_val))
                else:
                    print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_u_val: {:.3f}, constant_ics_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_val, self.adaptive_constant_bcs_h_val, self.adaptive_constant_ics_u_val, self.adaptive_constant_ics_h_val))
    ...
    def train_bfgs(self):
        ...
        if self.useObs:
            self.optimizer.minimize(self.sess,
                                    feed_dict=tf_dict,
                                    fetches=[self.loss, self.loss_f_c, self.loss_f_m, self.loss_BC_u, self.loss_BC_h, self.loss_IC_u, self.loss_IC_h, self.loss_obs_u, self.loss_obs_h],
                                    loss_callback=self.callback)
        else:
            self.optimizer.minimize(self.sess,
                                    feed_dict=tf_dict,
                                    fetches=[self.loss, self.loss_f_c, self.loss_f_m, self.loss_BC_u, self.loss_BC_h, self.loss_IC_u, self.loss_IC_h],
                                    loss_callback=self.callback)
    ...
    def save_weight(self, filepath):
        if self.useObs:
            weight_array = np.vstack([np.array(self.adpative_constant_bcs_u_log),
                                      ...
                                      np.array(self.adpative_constant_ics_u_log),
    ...
    def load_weight(self, filepath):
        ...
        if self.useObs:
            return weight_array[0,-1], weight_array[1,-1], weight_array[2,-1], weight_array[3,-1], weight_array[4,-1], weight_array[5,-1]
        else:
            return weight_array[0,-1], weight_array[1,-1], weight_array[2,-1], weight_array[3,-1] 
...
def l2_regularization(weights, lambda_reg=0.001):
    l2_loss = tf.add_n([tf.nn.l2_loss(w) for w in weights]) * lambda_reg
    return l2_loss
